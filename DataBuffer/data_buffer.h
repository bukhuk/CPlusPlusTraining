...
class DataBuffer {
  ...
  public:
    ...
    explicit DataBuffer(size_t capacity)
        : data_{new uint8_t[capacity]}, size_{0}, capacity_{capacity} {}
    ...
    template <typename... Args> void append_many(Args &&...args) {
        (append(std::forward<Args>(args)), ...);
    }

    template <typename T> auto view_as() {
        if (size_ % sizeof(T) != 0) {
            throw std::runtime_error("Buffer has remainder");
        }

        return BufferView<T>{reinterpret_cast<T *>(data_), reinterpret_cast<T *>(data_ + size_)};
    }
